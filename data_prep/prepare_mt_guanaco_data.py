...
for i, lang in enumerate(non_eng_langs):
    ...
    # outfile = f'data/guanaco_train_mt_ml{i+1}.json'
...
# 3. create ml LOO datasets with overlapping ids
for i, lang in enumerate(non_eng_langs):
    ...
    ml_df.to_json(f'data/guanaco_train_mtol_{lang}.json', orient='records', lines=True, force_ascii=False)
    print(f'wrote {len(ml_df)} rows to data/guanaco_train_mtol_{lang}.json')
